...
local function _4_()
  ...
  local function _5_(colors)
    _G.assert((nil ~= colors), "Missing argument colors on /Users/ck/.config/nvim/init.fnl:122")
...
later(_27_)
local function _28_(diagnostic)
  _G.assert((nil ~= diagnostic), "Missing argument diagnostic on /Users/ck/.config/nvim/init.fnl:276")
...
vim.api.nvim_create_autocmd("TextYankPost", {desc = "Highlight when yanking (copying) text", group = vim.api.nvim_create_augroup("kickstart-highlight-yank", {clear = true}), callback = _30_})
local function _31_()
  ...
  statusline.setup({content = {active = _32_, inactive = nil}})
  local function _33_()
    return "%2l:%-2v"
  end
  statusline.section_location = _33_
  return nil
end
now(_31_)
local function pick_marks(name, pattern)
  _G.assert((nil ~= pattern), "Missing argument pattern on /Users/ck/.config/nvim/init.fnl:350")
  _G.assert((nil ~= name), "Missing argument name on /Users/ck/.config/nvim/init.fnl:350")
  local pick = require("mini.pick")
...
local function _41_()
  ...
  map({"n", "x", "o", "v"}, "f", _42_, {desc = "Flash jump"})
  local function _43_()
    local flash0 = require("flash")
    return flash0.jump()
  end
  map({"i"}, "<c-f>", _43_, {desc = "Flash jump"})
  local function _44_()
    local flash0 = require("flash")
    return flash0.treesitter()
  end
  map({"n", "x", "o", "v"}, "F", _44_, {desc = "Flash treesitter jump"})
  local function _45_()
    local flash0 = require("flash")
    return flash0.toggle()
  end
  map("c", "<c-s>", _45_, {desc = "Flash toggle"})
  local function _46_()
    local flash0 = require("flash")
    return flash0.jump({continue = true})
  end
  map({"n", "x", "o"}, "<leader>jc", _46_, {desc = "Continue jump"})
  local function _47_()
    local flash0 = require("flash")
    return flash0.jump({search = {mode = "search", max_length = 0}, label = {after = {0, 0}}, pattern = "^"})
  end
  map({"n", "x", "o"}, "<leader>jl", _47_, {desc = "Jump to line"})
  local function _48_()
    local flash0 = require("flash")
    return flash0.jump({pattern = vim.fn.expand("<cword>")})
  end
  map({"n", "x", "o"}, "<leader>jw", _48_, {desc = "Jump to current word"})
  local function _49_()
    local flash0 = require("flash")
    local function _50_(matched, state)
      _G.assert((nil ~= state), "Missing argument state on /Users/ck/.config/nvim/init.fnl:489")
      _G.assert((nil ~= matched), "Missing argument matched on /Users/ck/.config/nvim/init.fnl:489")
      local function _51_()
        vim.api.nvim_win_set_cursor(matched.win, matched.pos)
        return vim.diagnostic.open_float()
      end
      vim.api.nvim_win_call(matched.win, _51_)
      return state:restore()
    end
    return flash0.jump({action = _50_})
  end
  return map({"n"}, "<leader>jd", _49_, {desc = "Show diagnostics at location"})
end
later(_41_)
local function _52_()
  local map = vim.keymap.set
  ...
  map("n", "<leader>ff", _53_, {desc = "Find files"})
  local function _54_()
    local pick = require("mini.pick")
    return pick.builtin.resume()
  end
  map("n", "<leader>f<enter>", _54_, {desc = "Resume"})
  local function _55_()
    local pick = require("mini.pick")
    return pick.builtin.grep_live()
  end
  map("n", "<leader><space>", _55_, {desc = "Find String"})
  local function _56_()
    local pick = require("mini.pick")
    return pick.builtin.buffers()
  end
  map("n", "<leader>fb", _56_, {desc = "Find Buffer"})
  local function _57_()
    local pick = require("mini.pick")
    local word = vim.fn.expand("<cword>")
    return pick.builtin.grep({pattern = word})
  end
  map("n", "<leader>fw", _57_, {desc = "Find Buffer"})
  local function _58_()
    local extra = require("mini.extra")
    return extra.pickers.buf_lines({scope = "current"})
  end
  map("n", ",", _58_, {desc = "Find Lines"})
  local function _59_()
    local ex = require("mini.extra")
    return ex.pickers.oldfiles({current_dir = true})
  end
  map("n", "<leader>fo", _59_, {desc = "Old files"})
  ...
  map("n", "<leader>fe", _61_, {desc = "Explore files"})
  local function _62_()
    return pick_marks("User marks", "[A-Za-z]")
  end
  map("n", "<leader>mm", _62_, {desc = "All user marks"})
  local function _63_()
    return pick_marks("Global marks", "[A-Z]")
  end
  map("n", "<leader>mg", _63_, {desc = "Global marks"})
  local function _64_()
    return pick_marks("Local marks", "[a-z]")
  end
  map("n", "<leader>ml", _64_, {desc = "Local user marks"})
  local function _65_()
    return pick_marks("Marks", ".")
  end
  map("n", "<leader>mM", _65_, {desc = "All marks"})
  local function _66_()
    return pick_marks("Marks", ".")
  end
  map("n", "<leader>ma", _66_, {desc = "All marks"})
  local function _67_()
    return vim.lsp.buf.definition()
  end
  map("n", "<leader>ld", _67_, {desc = "Go to definition"})
  local function _68_()
    return vim.lsp.buf.rename()
  end
  map("n", "<leader>lr", _68_, {desc = "Rename"})
  local function _69_()
    return vim.lsp.buf.code_action()
  end
  map("n", "<leader>la", _69_, {desc = "Code actions"})
  local function _70_()
    local extra = require("mini.extra")
    return extra.pickers.diagnostic({scope = "current"})
  end
  map("n", "<leader>le", _70_, {desc = "LSP Errors"})
  local function _71_()
    return vim.lsp.buf.definition()
  end
  map("n", "gd", _71_, {desc = "Go to definition"})
  local function _72_()
    return vim.lsp.buf.signature_help()
  end
  map("n", "<leader>lh", _72_, {desc = "Signature help"})
  local function _73_()
    local ex = require("mini.extra")
    return ex.pickers.lsp({scope = "references"})
  end
  map("n", "grr", _73_, {desc = "Go to references"})
  local function _74_()
    local ex = require("mini.extra")
    return ex.pickers.lsp({scope = "implementation"})
  end
  map("n", "gri", _74_, {desc = "Go to implementations"})
  local function _75_()
    local move = require("mini.move")
    return move.move_line("up")
  end
  map("n", "sk", _75_, {desc = "Move line up"})
  local function _76_()
    local move = require("mini.move")
    return move.move_line("down")
  end
  map("n", "sj", _76_, {desc = "Move line down"})
  local function _77_()
    local move = require("mini.move")
    return move.move_line("left")
  end
  map("n", "sh", _77_, {desc = "Move line up"})
  local function _78_()
    local move = require("mini.move")
    return move.move_line("right")
  end
  map("n", "sl", _78_, {desc = "Move line down"})
  local function _79_()
    local move = require("mini.move")
    return move.move_selection("up")
  end
  map("v", "sk", _79_, {desc = "Move selection up"})
  local function _80_()
    local move = require("mini.move")
    return move.move_selection("down")
  end
  map("v", "sj", _80_, {desc = "Move selection down"})
  local function _81_()
    local move = require("mini.move")
    return move.move_selection("left")
  end
  map("v", "sh", _81_, {desc = "Move selection left"})
  local function _82_()
    local move = require("mini.move")
    return move.move_selection("right")
  end
  map("v", "sl", _82_, {desc = "Move selection left"})
  local function _83_()
    return vim.diagnostic.open_float()
  end
  map("n", "<leader>dd", _83_, {desc = "Open diagnostic float"})
  local function _84_()
    local neogit = require("neogit")
    return neogit.open({kind = "floating"})
  end
  map("n", "<leader>gg", _84_, {desc = "Neo(g)it"})
  local function _85_()
    local notify = require("mini.notify")
    return notify.clear()
  end
  map("n", "<leader>nc", _85_, {desc = "Notify Clear"})
  local function _86_()
    local notify = require("mini.notify")
    return notify.show_history()
  end
  map("n", "<leader>nh", _86_, {desc = "Notify History"})
  local function _87_()
    local ex = require("mini.extra")
    return ex.pickers.visit_paths()
  end
  return map("n", "<leader>fr", _87_, {desc = "Visit paths"})
end
later(_52_)
local function _88_()
