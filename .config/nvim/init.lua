...
local function _20_()
  ...
  mini_notify.setup({lsp_progress = {level = "WARN"}})
...
vim.api.nvim_create_autocmd("TextYankPost", {desc = "Highlight when yanking (copying) text", group = vim.api.nvim_create_augroup("kickstart-highlight-yank", {clear = true}), callback = _31_})
local function _32_()
  ...
  statusline.setup({content = {active = _33_, inactive = nil}})
  local function _34_()
    return "%2l:%-2v"
  end
  statusline.section_location = _34_
  return nil
end
now(_32_)
...
local function _42_()
  ...
  map({"n", "x", "o", "v"}, "f", _43_, {desc = "Flash jump"})
  local function _44_()
    local flash0 = require("flash")
    return flash0.jump()
  end
  map({"i"}, "<c-f>", _44_, {desc = "Flash jump"})
  local function _45_()
    local flash0 = require("flash")
    return flash0.treesitter()
  end
  map({"n", "x", "o", "v"}, "F", _45_, {desc = "Flash treesitter jump"})
  local function _46_()
    local flash0 = require("flash")
    return flash0.toggle()
  end
  map("c", "<c-s>", _46_, {desc = "Flash toggle"})
  local function _47_()
    local flash0 = require("flash")
    return flash0.jump({continue = true})
  end
  map({"n", "x", "o"}, "<leader>jc", _47_, {desc = "Continue jump"})
  local function _48_()
    local flash0 = require("flash")
    return flash0.jump({search = {mode = "search", max_length = 0}, label = {after = {0, 0}}, pattern = "^"})
  end
  map({"n", "x", "o"}, "<leader>jl", _48_, {desc = "Jump to line"})
  local function _49_()
    local flash0 = require("flash")
    return flash0.jump({pattern = vim.fn.expand("<cword>")})
  end
  map({"n", "x", "o"}, "<leader>jw", _49_, {desc = "Jump to current word"})
  local function _50_()
    local flash0 = require("flash")
    local function _51_(matched, state)
      ...
      vim.api.nvim_win_call(matched.win, _52_)
      return state:restore()
    end
    return flash0.jump({action = _51_})
  end
  return map({"n"}, "<leader>jd", _50_, {desc = "Show diagnostics at location"})
end
later(_42_)
local function _53_()
  local map = vim.keymap.set
  ...
  map("n", "<leader>ff", _54_, {desc = "Find files"})
  local function _55_()
    local pick = require("mini.pick")
    return pick.builtin.resume()
  end
  map("n", "<leader>f<enter>", _55_, {desc = "Resume"})
  local function _56_()
    local pick = require("mini.pick")
    return pick.builtin.grep_live()
  end
  map("n", "<leader><space>", _56_, {desc = "Find String"})
  local function _57_()
    local pick = require("mini.pick")
    return pick.builtin.buffers()
  end
  map("n", "<leader>fb", _57_, {desc = "Find Buffer"})
  local function _58_()
    local pick = require("mini.pick")
    local word = vim.fn.expand("<cword>")
    return pick.builtin.grep({pattern = word})
  end
  map("n", "<leader>fw", _58_, {desc = "Find Buffer"})
  local function _59_()
    local extra = require("mini.extra")
    return extra.pickers.buf_lines({scope = "current"})
  end
  map("n", ",", _59_, {desc = "Find Lines"})
  local function _60_()
    local ex = require("mini.extra")
    return ex.pickers.oldfiles({current_dir = true})
  end
  map("n", "<leader>fo", _60_, {desc = "Old files"})
  ...
  map("n", "<leader>ee", _62_, {desc = "Explore files"})
  local function _63_()
    return pick_marks("User marks", "[A-Za-z]")
  end
  map("n", "<leader>mm", _63_, {desc = "All user marks"})
  local function _64_()
    return pick_marks("Global marks", "[A-Z]")
  end
  map("n", "<leader>mg", _64_, {desc = "Global marks"})
  local function _65_()
    return pick_marks("Local marks", "[a-z]")
  end
  map("n", "<leader>ml", _65_, {desc = "Local user marks"})
  local function _66_()
    return pick_marks("Marks", ".")
  end
  map("n", "<leader>mM", _66_, {desc = "All marks"})
  local function _67_()
    return pick_marks("Marks", ".")
  end
  map("n", "<leader>ma", _67_, {desc = "All marks"})
  local function _68_()
    return vim.lsp.buf.definition()
  end
  map("n", "<leader>ld", _68_, {desc = "Go to definition"})
  local function _69_()
    return vim.lsp.buf.rename()
  end
  map("n", "<leader>lr", _69_, {desc = "Rename"})
  local function _70_()
    return vim.lsp.buf.code_action()
  end
  map("n", "<leader>la", _70_, {desc = "Code actions"})
  local function _71_()
    local extra = require("mini.extra")
    return extra.pickers.diagnostic({scope = "current"})
  end
  map("n", "<leader>le", _71_, {desc = "LSP Errors"})
  local function _72_()
    return vim.lsp.buf.definition()
  end
  map("n", "gd", _72_, {desc = "Go to definition"})
  local function _73_()
    return vim.lsp.buf.signature_help()
  end
  map("n", "<leader>lh", _73_, {desc = "Signature help"})
  local function _74_()
    local ex = require("mini.extra")
    return ex.pickers.lsp({scope = "references"})
  end
  map("n", "grr", _74_, {desc = "Go to references"})
  local function _75_()
    local ex = require("mini.extra")
    return ex.pickers.lsp({scope = "implementation"})
  end
  map("n", "gri", _75_, {desc = "Go to implementations"})
  local function _76_()
    local move = require("mini.move")
    return move.move_line("up")
  end
  map("n", "sk", _76_, {desc = "Move line up"})
  local function _77_()
    local move = require("mini.move")
    return move.move_line("down")
  end
  map("n", "sj", _77_, {desc = "Move line down"})
  local function _78_()
    local move = require("mini.move")
    return move.move_line("left")
  end
  map("n", "sh", _78_, {desc = "Move line up"})
  local function _79_()
    local move = require("mini.move")
    return move.move_line("right")
  end
  map("n", "sl", _79_, {desc = "Move line down"})
  local function _80_()
    local move = require("mini.move")
    return move.move_selection("up")
  end
  map("v", "sk", _80_, {desc = "Move selection up"})
  local function _81_()
    local move = require("mini.move")
    return move.move_selection("down")
  end
  map("v", "sj", _81_, {desc = "Move selection down"})
  local function _82_()
    local move = require("mini.move")
    return move.move_selection("left")
  end
  map("v", "sh", _82_, {desc = "Move selection left"})
  local function _83_()
    local move = require("mini.move")
    return move.move_selection("right")
  end
  map("v", "sl", _83_, {desc = "Move selection left"})
  local function _84_()
    return vim.diagnostic.open_float()
  end
  map("n", "<leader>dd", _84_, {desc = "Open diagnostic float"})
  local function _85_()
    local neogit = require("neogit")
    return neogit.open({kind = "floating"})
  end
  map("n", "<leader>gg", _85_, {desc = "Neo(g)it"})
  local function _86_()
    local notify = require("mini.notify")
    return notify.clear()
  end
  map("n", "<leader>nc", _86_, {desc = "Notify Clear"})
  local function _87_()
    local notify = require("mini.notify")
    return notify.show_history()
  end
  map("n", "<leader>nh", _87_, {desc = "Notify History"})
  local function _88_()
    local ex = require("mini.extra")
    return ex.pickers.visit_paths()
  end
  return map("n", "<leader>fr", _88_, {desc = "Visit paths"})
end
return now(_53_)
