...
map("n", "<leader>ld", _11_, {desc = "Go to definition"})
local function _12_()
  return vim.lsp.buf.rename()
end
map("n", "<leader>lr", _12_, {desc = "Rename"})
local function _13_()
  return vim.lsp.buf.code_action()
end
map("n", "<leader>la", _13_, {desc = "Code actions"})
local function _14_()
  local extra = require("mini.extra")
  return extra.pickers.diagnostic({scope = "current"})
end
map("n", "<leader>le", _14_, {desc = "LSP Errors"})
local function _15_()
  return vim.lsp.buf.definition()
end
map("n", "gd", _15_, {desc = "Go to definition"})
local function _16_()
  return vim.lsp.buf.signature_help()
end
map("n", "<leader>lh", _16_, {desc = "Signature help"})
local function _17_()
  local ex = require("mini.extra")
  return ex.pickers.lsp({scope = "references"})
end
map("n", "grr", _17_, {desc = "Go to references"})
local function _18_()
  local ex = require("mini.extra")
  return ex.pickers.lsp({scope = "implementation"})
end
map("n", "gri", _18_, {desc = "Go to implementations"})
local function _19_()
  local ex = require("mini.extra")
  return ex.pickers.visit_paths()
end
return map("n", "<leader>fr", _19_, {desc = "Visit paths"})
